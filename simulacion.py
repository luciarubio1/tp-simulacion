...
ARQUITECTURA:
  ...
  1. Contiene toda la lógica de simulación (clases + funciones) corregida.
  2. Levanta un servidor HTTP embebido que sirve index.html, index.css, main.js
     y recibe parámetros en /simular devolviendo el resultado en JSON.

  Para correr: python simulacion.py
...
# ─────────────────────────────────────────────────────────────────────────────
#  SECCIÓN 1: RUNGE-KUTTA DE 4to ORDEN
#  La ecuación diferencial que modela los cortes eléctricos es:
#    dE/dt = (t^2 - E) * h^2
#  donde una unidad de t = 8 minutos.
#  Usamos RK4 para encontrar cuándo E supera E(0) por primera vez.
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
#  SECCIÓN 2: GENERADORES DE VARIABLES ALEATORIAS
# ─────────────────────────────────────────────────────────────────────────────
...
class Juez:
    def __init__(self, nombre):
        ...
        # Inicio de periodo de ocupacion
        self._inicio_periodo_ocupado = None


# ─────────────────────────────────────────────────────────────────────────────
#  SECCIÓN 4: MOTOR DE SIMULACIÓN PRINCIPAL
# ─────────────────────────────────────────────────────────────────────────────
...
def simular(params):
    ...
    def construir_fila(evento, detalles_evento=None, es_ultima=False):
        ...
        # Serializar competidores activos para interactividad
        ...
        # Configurar categoria de competidor en refuerzo de forma correcta
        if refuerzo.competidor_actual:
    ...
    while iteracion < MAX_ITER:
        ...
        if t_evento > T_MAX:
            reloj = T_MAX
            # Acumular tiempos de atención para jueces ocupados en el instante final
        ...
        # ── EVENTO: FIN TURNO REFUERZO ───────────────────────────────────────
            ...
            if len(cola) <= 1:
                # Si está ocupado, espera a terminar. Si está libre, se va ya.
        ...
        # ── EVENTO: CORTE ELÉCTRICO ──────────────────────────────────────────
            ...
            # Suspender evaluaciones (NO detenemos acumulador de ocupación, pues incluye cortes)
            for juez in [julian, enzo, refuerzo]:
        ...
        # ── EVENTO: FIN CORTE ELÉCTRICO ──────────────────────────────────────
            ...
            # Calcular próximo corte con RK4
            E0_nuevo = reloj
        ...
        # Construir y guardar fila si corresponde
        fila = construir_fila(evento_tipo, detalles)
        if ITER_DESDE <= iteracion < ITER_DESDE + ITER_CANT:
            vector_estado.append(fila)

    # Si salimos del loop sin fila final, agregarla sumando tiempos pendientes
...
# ─────────────────────────────────────────────────────────────────────────────
#  SERVIDOR HTTP EMBEBIDO (PARA SERVIR ESTÁTICOS)
# ─────────────────────────────────────────────────────────────────────────────
...
class SimulacionHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ruta_html = os.path.join(os.path.dirname(__file__), nombre_archivo)
